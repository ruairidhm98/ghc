...
class EventType:
    ...
    def __str__(self):
        return f'(n={self.n}, name={self.name})'
...
event_types = [
    ...
    EventType(27, 'ASSIGN_CAP',       [CapsetId, CapNo],              'Add capability to capability set'),
    EventType(28, 'REMOVE_CAP',       [CapsetId, CapNo],              'Remove capability from capability set'),
    ...
    EventType(207, 'CONC_NONMOVING_HEAP_CENSUS',   [Word8, Word32, Word32, Word32], 'Nonmoving heap census'),
]
...
def check_events() -> Dict[int, EventType]:
    ...
    for ty in event_types:
        if ty.n in seen_ids:
            print(f'Duplicate event type {ty.n}:')
            print(f'  {ty.name}')
            print(f'  {seen_ids[ty.n].name}')
            assert False
...
def generate_event_types_array() -> str:
    ...
    for ty in event_types:
        if ty.fields is VariableLength:
            ...
            length = ' + '.join(f'sizeof({field.c_type})'
                                for field in ty.fields)

        pr(f'    [EVENT_{ty.name}] = {{')
        pr(f'      .etNum = {ty.n},')
        pr(f'      .size = {length},')
        pr(f'      .desc = "{ty.description}"')
        pr(f'    }},')
...
def generate_event_types_defines() -> str:
    ...
    pr('')
    for ty in event_types:
        pr(f'#define EVENT_{ty.name} {ty.n}')
...
def main() -> None:
    ...
    parser.add_argument('--event-types-array', type=argparse.FileType('w'))
    parser.add_argument('--event-types-defines', type=argparse.FileType('w'))
