...
canCFunEqCan :: CtEvidence
             ...
             -> TcTyVar             -- RHS
             -> TcS (StopOrContinue Ct)
-- ^ Canonicalise a CFunEqCan.  We know that the function
-- on the LHS has already been flattened once, so we must
-- not flatten it again (lest we re-invent an fsk).
...
Wrinkles:
 ...
 (2) Though it would be sound to do so, we must not mark the rewritten Wanted
     ...
     (2b) We must now absolutely make sure to kick out any constraints that
          ...
          kickOutAfterFillingCoercionHole. Because we are doing this kick-out
          only because of these blocking coercion holes, we kick out only
          those constraints held up by a blocking coercion hole. This is
          recorded in the cc_status field of a CIrredCan. See kick_ct in
