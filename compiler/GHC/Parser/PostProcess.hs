...
mkStandaloneKindSig loc lhs rhs anns =
  do { vs <- mapM check_lhs_name (unLoc lhs)
     ...
     ; return $ L (noAnnSrcSpan loc)
       $ StandaloneKindSig (ApiAnn (realSrcSpan loc) anns cs) v rhs }
  where
    check_lhs_name v@(unLoc->name) =
      if isUnqual name && isTcOcc (rdrNameOcc name)
      then return v
      else addFatalError $ Error (ErrUnexpectedQualifiedConstructor (unLoc v)) [] (getLocA v)
...
mkTyFamInstEqn loc bndrs lhs rhs anns
  = do { (tc, tparams, fixity, ann) <- checkTyClHdr False lhs
       ...
       ; return (L (noAnnSrcSpan loc) $ FamEqn
                        { feqn_ext    = ApiAnn (realSrcSpan loc) (anns `mappend` ann) cs
                        , feqn_tycon  = tc
                        , feqn_bndrs  = bndrs
                        , feqn_pats   = tparams
                        , feqn_fixity = fixity
...
mkTyFamInst loc eqn anns = do
  ...
  return (L (noAnnSrcSpan loc) (TyFamInstD noExtField
              (TyFamInstDecl (ApiAnn (realSrcSpan loc) anns cs) eqn)))
...
mkSpliceDecl lexpr@(L loc expr)
  ...
  = L loc $ SpliceD noExtField (SpliceDecl noExtField (L loc splice) ExplicitSplice)
...
fromSpecTyVarBndr bndr = case bndr of
  ...
  where
    ...
    check_spec InferredSpec  loc = addFatalError $ Error ErrInferredTypeVarNotAllowed [] (locA loc)
...
cvBindsAndSigs fb = do
  ...
  where
    ...
    drop_bad_decls (L l (SpliceD _ d) : ds) = do
      addError $ Error (ErrDeclSpliceNotAtTopLevel d) [] (locA l)
      drop_bad_decls ds
...
tyConToDataCon :: LocatedN RdrName -> Either Error (LocatedN RdrName)
tyConToDataCon (L loc tc)
  ...
  | otherwise
  = Left $ Error (ErrNotADataCon tc) [] (locA loc)
  where
    occ = rdrNameOcc tc
...
mkGadtDecl loc names ty annsIn = do
  ...
  let (args, res_ty, annsa, csa)
      ...
      an = case outer_bndrs of
        _                -> ApiAnn (realSrcSpan loc) (annsIn ++ annsa) (cs ++ csa)
...
checkTyVars pp_what equals_or_where tc tparms
  ...
  where
    ...
    chk _ _ t@(L loc _)
        = addFatalError $ Error (ErrUnexpectedTypeInDecl t pp_what (unLoc tc) tparms equals_or_where) [] (locA loc)
...
checkDatatypeContext (Just c)
    = do allowed <- getBit DatatypeContextsBit
         unless allowed $ addError $ Error (ErrIllegalDataTypeContext c) [] (getLocA c)
...
checkRuleTyVarBndrNames = mapM_ (check . fmap hsTyVarName)
  where check (L loc (Unqual occ)) =
          ...
          when ((occNameString occ ==) `any` ["forall","family","role"])
            (addFatalError $ Error (ErrParseErrorOnInput occ) [] (locA loc))
        check _ = panic "checkRuleTyVarBndrNames"

checkRecordSyntax :: (MonadP m, Outputable a) => LocatedA a -> m (LocatedA a)
checkRecordSyntax lr@(L loc r)
    = do allowed <- getBit TraditionalRecordSyntaxBit
         unless allowed $ addError $ Error (ErrIllegalTraditionalRecordSyntax (ppr r)) [] (locA loc)
         return lr
...
checkTyClHdr is_cls ty
  ...
  where
    ...
    go _ (HsParTy an (L l (HsStarTy _ isUni))) acc ann' fix
      = do { addWarning Opt_WarnStarBinder (WarnStarBinder (locA l))
    ...
    newAnns :: SrcSpanAnnA -> ApiAnn' AnnParen -> SrcSpanAnnName
    newAnns (SrcSpanAnn ApiAnnNotUsed l) (ApiAnn as (AnnParen _ o c) cs) =
      let
        lr = combineRealSrcSpans (realSrcSpan l) as
        an = (ApiAnn lr (NameAnn NameParens o (realSrcSpan l) c []) cs)
    ...
    newAnns (SrcSpanAnn (ApiAnn ap (AnnListItem ta) csp) l) (ApiAnn as (AnnParen _ o c) cs) =
      let
        lr = combineRealSrcSpans ap as
        an = (ApiAnn lr (NameAnn NameParens o (realSrcSpan l) c ta) (csp ++ cs))
      in SrcSpanAnn an (RealSrcSpan lr Nothing)
  ...
  where
    ...
    check err a = do
      ...
      unless blockArguments $
        addError $ Error (err a) [] (getLocA a)
 ...
 where
  ...
  check (oparens,cparens,cs) (L _l (HsTupleTy ann' HsBoxedOrConstraintTuple ts))
    ...
    = do
        ...
        return (L (SrcSpanAnn (ApiAnn (realSrcSpan l)
...
checkAPat loc e0 = do
 ...
 case e0 of
   ...
   PatBuilderOpApp _ op _ _ | opIsAt (unLoc op) -> do
     addError $ Error ErrAtInPatPos [] (getLocA op)
   ...
   PatBuilderPar e an@(AnnParen pt o c) -> do
     ...
     return (ParPat (ApiAnn (realSrcSpan $ (widenSpan (locA l) aa)) an []) (L l p))
...
checkPatBind :: SrcSpan
             ...
             -> Located (GRHSs GhcPs (LHsExpr GhcPs))
             -> P (HsBind GhcPs)
checkPatBind loc annsIn lhs (L _match_span grhss)
...
checkValSigLhs lhs@(L l _)
  = addFatalError $ Error (ErrInvalidTypeSignature lhs) [] (locA l)
...
instance DisambInfixOp RdrName where
  ...
  mkHsInfixHolePV l _ = addFatalError $ Error ErrInvalidInfixHole [] l
...
instance DisambECP (HsCmd GhcPs) where
  ...
  mkHsDoPV l (Just m)    _ _ = addFatalError $ Error (ErrQualifiedDoInCmd m) [] l
...
instance DisambECP (HsExpr GhcPs) where
  ...
  ecpFromCmd' (L l c) = do
    addError $ Error (ErrArrowCmdInExpr c) [] (locA l)
  ...
  mkHsTySigPV l a sig anns = do
    cs <- getCommentsFor (locA l)
    return $ L l (ExprWithTySig (ApiAnn (realSrcSpan $ locA l) anns cs) a (hsTypeToHsSigWcType sig))
  ...
  mkHsViewPatPV l a b _ = addError (Error (ErrViewPatInExpr a b) [] l)
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkHsAsPatPV l v e   _ = addError (Error (ErrTypeAppWithoutSpace (unLoc v) e) [] l)
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkHsLazyPatPV l e   _ = addError (Error (ErrLazyPatWithoutSpace e) [] l)
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkHsBangPatPV l e   _ = addError (Error (ErrBangPatWithoutSpace e) [] l)
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkSumOrTuplePV = mkSumOrTupleExpr
  rejectPragmaPV (L _ (OpApp _ _ _ e)) =
    -- assuming left-associative parsing of operators
    rejectPragmaPV e
  rejectPragmaPV (L l (HsPragE _ prag _)) = addError $ Error (ErrUnallowedPragma prag) [] (locA l)
  rejectPragmaPV _                        = return ()
...
instance DisambECP (PatBuilder GhcPs) where
  ...
  ecpFromCmd' (L l c)    = addFatalError $ Error (ErrArrowCmdInPat c) [] (locA l)
  ecpFromExp' (L l e)    = addFatalError $ Error (ErrArrowExprInPat e) [] (locA l)
  mkHsLamPV l _          = addFatalError $ Error ErrLambdaInPat [] l
  mkHsLetPV l _ _ _      = addFatalError $ Error ErrLetInPat [] l
  ...
  mkHsCasePV l _ _ _     = addFatalError $ Error ErrCaseInPat [] l
  mkHsLamCasePV l _ _    = addFatalError $ Error ErrLambdaCaseInPat [] l
  type FunArg (PatBuilder GhcPs) = PatBuilder GhcPs
  superFunArg m = m
  mkHsAppPV l p1 p2     = return $ L l (PatBuilderApp p1 p2)
  mkHsAppTypePV l _ _ _ = addFatalError $ Error ErrTypeAppInPat [] (locA l)
  mkHsIfPV l _ _ _ _ _ _= addFatalError $ Error ErrIfTheElseInPat [] l
  mkHsDoPV l _ _ _      = addFatalError $ Error ErrDoNotationInPat [] l
...
mkPatRec p _ _ =
  addFatalError $ Error (ErrInvalidRecordCon (unLoc p)) [] (getLocA p)
...
instance DisambTD DataConBuilder where
  ...
  mkHsOpTyPV lhs tc rhs = do
    ...
    where
      ...
      check_no_ops (HsOpTy{}) =
        addError $ Error (ErrInvalidInfixDataCon (unLoc lhs) (unLoc tc) (unLoc rhs)) [] (locA l)
...
tyToDataConBuilder t =
  addFatalError $ Error (ErrInvalidDataCon (unLoc t)) [] (getLocA t)
...
mkRecConstrOrUpdate exp _ (fs,dd) anns
  ...
  | otherwise = return (mkRdrRecordUpd exp (map (fmap mk_rec_upd_field) fs) anns)
         ...
         if allowed
          then
            ...
            in (\newName
               ...
               <$> nameT
          else addFatalError $ Error ErrIllegalPatSynExport [] (locA l)
  where
    name = ieNameVal specname
    nameT =
      if isVarNameSpace (rdrNameSpace name)
        then addFatalError $ Error (ErrVarForTyCon name) [] (locA l)
...
mkTypeImpExp name =
  do allowed <- getBit ExplicitNamespacesBit
     unless allowed $ addError $ Error ErrIllegalExplicitNamespace [] (getLocA name)
...
failOpFewArgs (L loc op) =
  do { star_is_type <- getBit StarIsTypeBit
     ; addFatalError $ Error (ErrOpFewArgs (StarIsType star_is_type) op) [] (locA loc) }
...
mkSumOrTupleExpr l Unboxed (Sum alt arity e barsp barsa) anns = do
    ...
    cs <- getCommentsFor (locA l)
    return $ L l (ExplicitSum (ApiAnn (realSrcSpan $ locA l) an cs) alt arity e)
mkSumOrTupleExpr l Boxed a@Sum{} _ =
    addFatalError $ Error (ErrUnsupportedBoxedSumExpr a) [] (locA l)
...
mkSumOrTuplePat l boxity (Tuple ps) anns = do
  ...
  where
    ...
    toTupPat p = case p of
      Left _ -> addFatalError $ Error ErrTupleSectionInPat [] (locA l)
      Right p' -> checkLPat p'
...
mkSumOrTuplePat l Unboxed (Sum alt arity p barsb barsa) anns = do
   p' <- checkLPat p
   cs <- getCommentsFor (locA l)
   let an = ApiAnn (realSrcSpan $ locA l) (ApiAnnSumPat anns barsb barsa) cs
   return $ L l (PatBuilderPat (SumPat an p' alt arity))
mkSumOrTuplePat l Boxed a@Sum{} _ =
    addFatalError $ Error (ErrUnsupportedBoxedSumPat a) [] (locA l)
