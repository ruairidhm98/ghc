...
semis1  :: { [TrailingAnn] }
semis1  : semis1 ';'  { if isZeroWidthSpan (gl $2) then $1 else (AddSemiAnn (glR $2) : $1) }
        | ';'         { msemi $1 }
                ...
                {% do {
                  ...
                  ; let anns
                         = ApiAnnImportDecl
                             ...
                             , importDeclAnnQualified = fst $ importDeclQualifiedStyle $4 $7
                             , importDeclAnnPackage   = fst $5
                             , importDeclAnnAs        = fst $8
                             }
                  ; fmap reLocA $ acs (\cs -> L (comb4 $1 $6 (snd $8) $9) $
                      ImportDecl { ideclExt = ApiAnn (glR $1) anns cs
                                  , ideclSourceSrc = snd $ fst $2
                                  , ideclName = $6, ideclPkgQual = snd $5
                                  , ideclSource = snd $2, ideclSafe = snd $3
                                  , ideclQualified = snd $ importDeclQualifiedStyle $4 $7
                                  , ideclImplicit = False
                                  , ideclAs = unLoc (snd $8)
                                  , ideclHiding = unLoc $9 })
                  }
...
maybe_src :: { ((Maybe (RealSrcSpan,RealSrcSpan),SourceText),IsBootInterface) }
        : '{-# SOURCE' '#-}'        { ((Just (glR $1,glR $2),getSOURCE_PRAGs $1)
                                      , IsBoot) }
        | {- empty -}               { ((Nothing,NoSourceText),NotBoot) }

maybe_safe :: { (Maybe RealSrcSpan,Bool) }
        : 'safe'                                { (Just (glR $1),True) }
        | {- empty -}                           { (Nothing,      False) }

maybe_pkg :: { (Maybe RealSrcSpan,Maybe StringLiteral) }
...
where_decls :: { LocatedL (OrdList (LHsDecl GhcPs)) }
        : 'where' '{' decls '}'       {% amsrl (sLL $1 $> (snd $ unLoc $3))
                                              (AnnList (Just $ moc $2) (Just $ mcc $4) [mj AnnWhere $1] (fst $ unLoc $3)) }
        | 'where' vocurly decls close {% amsrl (sLL $1 $3 (snd $ unLoc $3))
                                              (AnnList Nothing Nothing [mj AnnWhere $1] (fst $ unLoc $3))}
...
decllist :: { Located (AnnList,Located (OrdList (LHsDecl GhcPs))) }
        : '{'            decls '}'     { sLL $1 $> (AnnList (Just $ moc $1) (Just $ mcc $3) [] (fst $ unLoc $2)
                                                   ,sL1 $2 $ snd $ unLoc $2) }
        |     vocurly    decls close   { L (gl $2) (AnnList Nothing Nothing [] (fst $ unLoc $2)
                                                   ,sL1 $2 $ snd $ unLoc $2) }
...
atype :: { LHsType GhcPs }
        ...
        | '{' fielddecls '}'             {% do { decls <- acsA (\cs -> (sLL $1 $> $ HsRecTy (ApiAnn (glR $1) (AnnList (Just $ moc $1) (Just $ mcc $3) [] []) cs) $2))
...
gdrh :: { LGRHS GhcPs (LHsExpr GhcPs) }
        : '|' guardquals '=' exp  {% runPV (unECP $4) >>= \ $4 ->
                                     acs (\cs -> sL (comb2A $1 $>) $ GRHS (ApiAnn (glR $1) (GrhsAnn (Just $ glR $1) (mj AnnEqual $3)) cs) (unLoc $2) $4) }
        ...
        | DO  stmtlist               {% do
                                      ...
                                      return $ ECP $
                                        ...
                                        mkHsDoPV (comb2A $1 $2)
                                                 ...
                                                 $2
                                                 (AnnList Nothing Nothing [mj AnnDo $1] []) }
        | MDO stmtlist             {% hintQualifiedDo $1 >> runPV $2 >>= \ $2 ->
                                       ...
                                       acsA (\cs -> L (comb2A $1 $2)
                                              (mkHsDoAnns (MDoExpr $
                                                          ...
                                                          $2
                                           (ApiAnn (glR $1) (AnnList Nothing Nothing [mj AnnMdo $1] []) cs) )) }
        | 'proc' aexp '->' exp
...
aexp2   :: { ECP }
        ...
        | ipvar %shift                  {% acsExpr (\cs -> sL1a $1 (HsIPVar (comment (glR $1) cs) $! unLoc $1)) }
...
--                                       (getSTRING $1) noExtField) }
        ...
        | '(' texp ')'                  { ECP $
                                           unECP $2 >>= \ $2 ->
                                           mkHsParPV (comb2 $1 $>) $2 (AnnParen AnnParens (glR $1) (glR $3)) }
        | '(' tup_exprs ')'             { ECP $
                                           $2 >>= \ $2 ->
        ...
        | '(|' aexp cmdargs '|)'  {% runPV (unECP $2) >>= \ $2 ->
                                      fmap ecpFromCmd $
                                      acsA (\cs -> sLL $1 $> $ HsCmdArrForm (ApiAnn (glR $1) (AnnList (Just $ mu AnnOpenB $1) (Just $ mu AnnCloseB $4) [] []) cs) $2 Prefix
                                                           Nothing (reverse $3)) }
...
tup_exprs :: { forall b. DisambECP b => PV (SumOrTuple b) }
           ...
           | commas tup_tail
                 { $2 >>= \ $2 ->
                   do { let {cos = map (\ll -> (Left (ApiAnn (rs ll) (rs ll) []))) (fst $1) }
                      ; return (Tuple (cos ++ $2)) } }
...
commas_tup_tail : commas tup_tail
        { $2 >>= \ $2 ->
          do { let {cos = map (\l -> (Left (ApiAnn (rs l) (rs l) []))) (tail $ fst $1) }
             ; return ((head $ fst $1, cos ++ $2)) } }

-- Always follows a comma
tup_tail :: { forall b. DisambECP b => PV [Either (ApiAnn' RealSrcSpan) (LocatedA b)] }
...
list :: { forall b. DisambECP b => SrcSpan -> (AddApiAnn, AddApiAnn) -> PV (LocatedA b) }
        : texp    { \loc (ao,ac) -> unECP $1 >>= \ $1 ->
                            mkHsExplicitListPV loc [$1] (AnnList (Just ao) (Just ac) [] []) }
        | lexps   { \loc (ao,ac) -> $1 >>= \ $1 ->
                            mkHsExplicitListPV loc (reverse $1) (AnnList (Just ao) (Just ac) [] []) }
        | texp '..'  { \loc (ao,ac) -> unECP $1 >>= \ $1 ->
                                  acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [ao,mj AnnDotdot $2,ac] cs) Nothing (From $1))
                                      >>= ecpFromExp' }
        | texp ',' exp '..' { \loc (ao,ac) ->
                                   unECP $1 >>= \ $1 ->
                                   unECP $3 >>= \ $3 ->
                                   acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [ao,mj AnnComma $2,mj AnnDotdot $4,ac] cs) Nothing (FromThen $1 $3))
                                       >>= ecpFromExp' }
        | texp '..' exp  { \loc (ao,ac) ->
                                   unECP $1 >>= \ $1 ->
                                   unECP $3 >>= \ $3 ->
                                   acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [ao,mj AnnDotdot $2,ac] cs) Nothing (FromTo $1 $3))
                                       >>= ecpFromExp' }
        | texp ',' exp '..' exp { \loc (ao,ac) ->
                                   unECP $1 >>= \ $1 ->
                                   unECP $3 >>= \ $3 ->
                                   unECP $5 >>= \ $5 ->
                                   acsA (\cs -> L loc $ ArithSeq (ApiAnn (realSrcSpan loc) [ao,mj AnnComma $2,mj AnnDotdot $4,ac] cs) Nothing (FromThenTo $1 $3 $5))
                                       >>= ecpFromExp' }
        | texp '|' flattenedpquals
             { \loc (ao,ac) ->
                checkMonadComp >>= \ ctxt ->
                unECP $1 >>= \ $1 -> do { t <- addTrailingVbarA $1 (gl $2)
                ; acsA (\cs -> L loc $ mkHsCompAnns ctxt (unLoc $3) t (ApiAnn (realSrcSpan loc) (AnnList (Just ao) (Just ac) [] []) cs))
                    >>= ecpFromExp' } }
...
squals :: { Located [LStmt GhcPs (LHsExpr GhcPs)] }   -- In reverse order, because the last
    ...
    : squals ',' transformqual
             {% case unLoc $1 of
                  (h:t) -> do
                    h' <- addTrailingCommaA h (gl $2)
                    return (sLL $1 $> [sLLa $1 $> ((unLoc $3) (glR $1) (reverse (h':t)))]) }
    | squals ',' qual
             {% runPV $3 >>= \ $3 ->
                case unLoc $1 of
                  (h:t) -> do
    ...
    | transformqual        {% return (sLL $1 $> [L (getLocAnn $1) ((unLoc $1) (glR $1) [])]) }
    | qual                               {% runPV $1 >>= \ $1 ->
                                            return $ sL1A $1 [$1] }
...
transformqual :: { Located (RealSrcSpan -> [LStmt GhcPs (LHsExpr GhcPs)] -> Stmt GhcPs (LHsExpr GhcPs)) }
    ...
    : 'then' exp              {% runPV (unECP $2) >>= \ $2 ->
                                 acs (\cs->
                                 sLLlA $1 $> (\r ss -> (mkTransformStmt (ApiAnn r [mj AnnThen $1] cs) ss $2))) }
    | 'then' exp 'by' exp     {% runPV (unECP $2) >>= \ $2 ->
                                 runPV (unECP $4) >>= \ $4 ->
                                 acs (\cs -> sLLlA $1 $> (
                                                     \r ss -> (mkTransformByStmt (ApiAnn r [mj AnnThen $1,mj AnnBy $3] cs) ss $2 $4))) }
    | 'then' 'group' 'using' exp
            {% runPV (unECP $4) >>= \ $4 ->
               acs (\cs -> sLLlA $1 $> (
                                   \r ss -> (mkGroupUsingStmt (ApiAnn r [mj AnnThen $1,mj AnnGroup $2,mj AnnUsing $3] cs) ss $4))) }

    | 'then' 'group' 'by' exp 'using' exp
            {% runPV (unECP $4) >>= \ $4 ->
               runPV (unECP $6) >>= \ $6 ->
               acs (\cs -> sLLlA $1 $> (
                                   \r ss -> (mkGroupByUsingStmt (ApiAnn r [mj AnnThen $1,mj AnnGroup $2,mj AnnBy $3,mj AnnUsing $5] cs) ss $4 $6))) }
...
altslist :: { forall b. DisambECP b => PV (LocatedL [LMatch GhcPs (LocatedA b)]) }
        : '{'            alts '}'  { $2 >>= \ $2 -> amsrl
                                     (sLL $1 $> (reverse (snd $ unLoc $2)))
                                               (AnnList (Just $ moc $1) (Just $ mcc $3) (fst $ unLoc $2) []) }
        |     vocurly    alts  close { $2 >>= \ $2 -> amsrl
                                       (L (getLoc $2) (reverse (snd $ unLoc $2)))
                                        (AnnList Nothing Nothing (fst $ unLoc $2) []) }
        | '{'                 '}'    { amsrl (sLL $1 $> []) (AnnList (Just $ moc $1) (Just $ mcc $2) [] []) }
...
gdpat   :: { forall b. DisambECP b => PV (LGRHS GhcPs (LocatedA b)) }
        : '|' guardquals '->' exp
                                   { unECP $4 >>= \ $4 ->
                                     acs (\cs -> sL (comb2A $1 $>) $ GRHS (ApiAnn (glR $1) (GrhsAnn (Just $ glR $1) (mu AnnRarrow $3)) cs) (unLoc $2) $4) }
...
stmtlist :: { forall b. DisambECP b => PV (LocatedL [LStmt GhcPs (LocatedA b)]) }
        : '{'           stmts '}'       { $2 >>= \ $2 -> amsrl
                                          (sLL $1 $> (reverse $ snd $ unLoc $2)) (AnnList (Just $ moc $1) (Just $ mcc $3) [] (fst $ unLoc $2)) } -- AZ:performance of reverse?
        |     vocurly   stmts close     { $2 >>= \ $2 -> amsrl
                                          (L (gl $2) (reverse $ snd $ unLoc $2)) (AnnList Nothing Nothing [] (fst $ unLoc $2)) }
...
stmt  :: { forall b. DisambECP b => PV (LStmt GhcPs (LocatedA b)) }
        ...
        | 'rec' stmtlist                {  $2 >>= \ $2 ->
                                           acsA (\cs -> (sLL $1 (reLoc $>) $ mkRecStmt
                                                 (ApiAnn (glR $1) (hsDoAnn $1 AnnRec) cs)
                                                  $2)) }
...
hole_op : '`' '_' '`'           { mkHsInfixHolePV (comb2 $1 $>)
                                         (\cs -> ApiAnn (glR $1) (ApiAnnUnboundVar (glR $1, glR $3) (glR $2)) cs) }
...
bars0 :: { ([RealSrcSpan],Int) }     -- Zero or more bars
        : bars                   { $1 }
        |                        { ([], 0) }

bars :: { ([RealSrcSpan],Int) }     -- One or more bars
        : bars '|'               { ((fst $1)++[glR $2],snd $1 + 1) }
        | '|'                    { ([glR $1],1) }
...
msemi l = if isZeroWidthSpan (gl l) then [] else [AddSemiAnn (rs $ gl l)]
...
addTrailingVbarL  la span = addTrailingAnnL la (AddVbarAnn (rs span))

addTrailingCommaL :: MonadP m => LocatedL a -> SrcSpan -> m (LocatedL a)
addTrailingCommaL  la span = addTrailingAnnL la (AddCommaAnn (rs span))
...
addTrailingDarrowC (L (SrcSpanAnn ApiAnnNotUsed l) a) lt cs =
  ...
  in L (SrcSpanAnn (ApiAnn (realSrcSpan l) (AnnContext (Just (u,glR lt)) [] []) cs) l) a
addTrailingDarrowC (L (SrcSpanAnn (ApiAnn lr (AnnContext _ o c) csc) l) a) lt cs =
  let
    u = if (isUnicode lt) then UnicodeSyntax else NormalSyntax
  in L (SrcSpanAnn (ApiAnn lr (AnnContext (Just (u,glR lt)) o c) (cs++csc)) l) a
